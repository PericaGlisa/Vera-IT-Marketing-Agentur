...
export function ContactForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const emailSubject = `Neue Projektanfrage von ${formData.name}`
      const emailBody = `
Name: ${formData.name}
...
Gesendet über das Kontaktformular von verait.de
      ...
      // Create mailto link
      const mailtoLink = `mailto:info@verait.de?subject=${encodeURIComponent(emailSubject)}&body=${encodeURIComponent(emailBody)}`
      
      // Open email client
      window.location.href = mailtoLink
      
      // Show success message
      alert('Ihr E-Mail-Client wird geöffnet. Bitte senden Sie die E-Mail ab, um Ihre Anfrage zu übermitteln.')
